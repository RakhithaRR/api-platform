...
// ackResourceTypeAgent is the resourceType an Agent deployment acknowledgement
// reports under, matching the "agent.*" event family the control plane sends.
const ackResourceTypeAgent = "agent"
...
func (c *Client) handleAgentDeployedEvent(event map[string]any) {
	...
	agentID := deployedEvent.Payload.AgentID
	if agentID == "" {
	...
	result, err := c.agentService.Create(agent.CreateParams{
		Body:          yamlData,
		ContentType:   "application/yaml",
		ID:            deployAgentID,
		DeploymentID:  deployedEvent.Payload.DeploymentID,
		Origin:        models.OriginControlPlane,
		DeployedAt:    &agentPerformedAt,
		CorrelationID: deployedEvent.CorrelationID,
		Logger:        c.logger,
	})
	if err != nil {
...
func (c *Client) handleAgentUndeployedEvent(event map[string]any) {
	...
	agentID := undeployedEvent.Payload.AgentID
	if agentID == "" {
...
func (c *Client) handleAgentDeletedEvent(event map[string]any) {
	...
	agentID := deletedEvent.Payload.AgentID
	if agentID == "" {
