...
func (c *Client) processSyncFetchBatch(batch []models.ControlPlaneDeployment, gatewayID string) {
	...
	for _, dep := range batch {
		...
		case models.KindAgent:
			...
			// Called directly rather than through an apiUtilsService wrapper: the
			// Agent service already takes the deployment identity as parameters,
			// so a Create*FromYAML shim would only re-wrap them.
			_, err = c.agentService.Create(agent.CreateParams{
				Body:          yamlData,
