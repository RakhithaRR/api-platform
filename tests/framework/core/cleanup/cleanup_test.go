...
func TestCleanupScopesHaveIndependentLifetimes(t *testing.T) {
	rec := newRecorder()
	r := registryWith(rec, KindAPI)

	require.NoError(t, r.Register(Resource{Kind: KindAPI, ID: "scenario-api", Actor: "admin"}))
	require.NoError(t, r.RegisterRunner(Resource{Kind: KindAPI, ID: "fixture-api", Actor: "admin"}))

	require.NoError(t, r.Sweep(context.Background()))
	require.Equal(t, []string{"api:scenario-api"}, rec.order())
	require.Equal(t, 1, r.Count(), "runner resources must survive scenario cleanup")

	require.NoError(t, r.SweepRunner(context.Background()))
	require.Equal(t, []string{"api:scenario-api", "api:fixture-api"}, rec.order())
	require.Zero(t, r.Count())
}

func TestCleanupScopeRejectsUnknownValue(t *testing.T) {
	r := NewRegistry(nil)
	err := r.RegisterScoped(Scope(99), Resource{Kind: KindAPI, ID: "api-1", Actor: "admin"})
	require.ErrorContains(t, err, "invalid resource scope")
	require.Zero(t, r.Count())
}

func TestRunnerRegistrationPromotesExistingScenarioResource(t *testing.T) {
	rec := newRecorder()
	r := registryWith(rec, KindAPI)
	resource := Resource{Kind: KindAPI, ID: "fixture-api", Actor: "admin"}
	require.NoError(t, r.Register(resource))
	require.NoError(t, r.RegisterRunner(resource))

	require.NoError(t, r.Sweep(context.Background()))
	require.Equal(t, 1, r.Count(), "promotion must protect the runner-owned resource")
	require.NoError(t, r.SweepRunner(context.Background()))
	require.Equal(t, []string{"api:fixture-api"}, rec.order())
}

func TestCrossScopeDuplicateIsDeletedOnce(t *testing.T) {
	rec := newRecorder()
	r := registryWith(rec, KindAPI)
	resource := Resource{Kind: KindAPI, ID: "api-1", Actor: "admin"}
	require.NoError(t, r.RegisterRunner(resource))
...
func TestContextCleanupScopes(t *testing.T) {
	rec := newRecorder()
	r := registryWith(rec, KindAPI)
	ctx := runnerCtx()
	require.NoError(t, Install(ctx, r))
	require.NoError(t, Register(ctx, Resource{Kind: KindAPI, ID: "scenario-api", Actor: "admin"}))
	require.NoError(t, RegisterRunner(ctx, Resource{Kind: KindAPI, ID: "fixture-api", Actor: "admin"}))

	require.NoError(t, Sweep(ctx))
	require.Equal(t, 1, r.Count())
	require.NoError(t, SweepRunner(ctx))
	require.Zero(t, r.Count())
}
